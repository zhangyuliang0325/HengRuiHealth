...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger y = 13 + _chartCavanHeight;

            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(x, y, (NSInteger) _xLabelWidth, (NSInteger) _chartMarginBottom + 3)];
...
//    [chartPath removeAllObjects];
//    [pathPoints removeAllObjects];
//    [pointsOfPath removeAllObjects];
    // Draw each line
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        PNLineChartData *chartData = self.chartData[lineIndex];
...
//            int x = i * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0;
            //自定义x
            int xIndex = [chartData.xDatas[i] intValue];
            float x = xIndex * self.xLabelWidth  + _chartMarginLeft + _xLabelWidth / 2.0;
//            float x = xIndex * self.xLabelWidth + self.chartMarginLeft;

//            int y = _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2) + _chartMarginTop - _chartMarginBottom;
            //自定义Y
            float y = self.bounds.size.height - (yValue * self.yLabelHeight + self.chartMarginBottom + 10) + 10;
...
//                    float y1 = y - (inflexionWidth / 2) / distance * (y - last_y);
                    ...
                    int xIndex1 = [chartData.xDatas[i - 1] intValue];
                    float last_x1 = xIndex1 * self.xLabelWidth  + _chartMarginLeft + _xLabelWidth / 2.0 + (inflexionWidth / 2);
//                    float last_x1 = xIndex * self.xLabelWidth + self.chartMarginLeft;
                    float yValue1 = chartData.getData(i - 1).y;
                    float last_y1 = self.bounds.size.height - (yValue1 * self.yLabelHeight + self.chartMarginBottom + 10) + 10;
...
//            point = CGPointMake(_chartMarginLeft + yAxisOffset, (_chartCavanHeight - i * yStepHeight + _yLabelHeight / 2));
            point = CGPointMake(_chartMarginLeft, (_chartCavanHeight - i * yStepHeight + _yLabelHeight / 2) + 10);
            CGContextMoveToPoint(ctx, point.x, point.y);
...
//            UIView *line = [[UIView alloc] initWithFrame:CGRectMake(x + legendLineWidth * 0.1, y + (singleRowHeight - pdata.lineWidth) / 2, halfLineLength, pdata.lineWidth)];
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - labelsize.width - 8 , y, labelsize.width, labelsize.height)];
            label.text = pdata.dataTitle;
            label.textColor = self.legendFontColor ? self.legendFontColor : [UIColor blackColor];
            label.font = self.legendFont ? self.legendFont : [UIFont systemFontOfSize:12.0f];
            label.lineBreakMode = NSLineBreakByWordWrapping;
            ...
            UIImageView *inflex = [self drawInflexion:pdata.inflexionPointWidth
                         center:CGPointMake([UIScreen mainScreen].bounds.size.width - labelsize.width - 10 - halfLineLength - pdata.inflexionPointWidth, y + singleRowHeight / 2)
                    strokeWidth:pdata.lineWidth
                 inflexionStyle:pdata.inflexionPointStyle
                       andColor:inflexionPointColor
                       andAlpha:pdata.alpha];
            [legendViews addObject:inflex];
            
//            [label sizeToFit];
//            label.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 8 - label.bounds.size.width, y, labelsize.width, label.bounds.size.width, labelsize.height);
            
            UIView *line = [[UIView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 10 - labelsize.width - halfLineLength - pdata.lineWidth, y + (singleRowHeight - pdata.lineWidth) / 2, halfLineLength, pdata.lineWidth)];
            line.backgroundColor = pdata.color;
//            line.backgroundColor = [UIColor greenColor];
            ...
            if (pdata.inflexionPointStyle != PNLineChartPointStyleNone) {
//                line = [[UIView alloc] initWithFrame:CGRectMake(x + legendLineWidth * 0.1 + halfLineLength + inflexionWidthSpacer, y + (singleRowHeight - pdata.lineWidth) / 2, halfLineLength, pdata.lineWidth)];
                line = [[UIView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 10 -  2 * halfLineLength - inflex.bounds.size.width - labelsize.width, y + (singleRowHeight - pdata.lineWidth) / 2, halfLineLength, pdata.lineWidth)];
                line.backgroundColor = pdata.color;
